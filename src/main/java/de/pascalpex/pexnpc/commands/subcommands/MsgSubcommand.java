...
public class MsgSubcommand implements Command<CommandSourceStack> {
    @Override
    public int run(CommandContext<CommandSourceStack> context) {
        ...
        placeableNPC.getNpc().setMessage(msg);
        NPCData.saveNpc(placeableNPC.getNpc());

        sender.sendMessage(MessageHandler.prefixedMini("The NPC with the ID <gold>" + placeableNPC.getNpc().getId() + " <aqua>now has the message ").append(MessageHandler.parseAmpersand(msg)));
